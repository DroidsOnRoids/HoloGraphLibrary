...
public class PieGraphAnimated extends SurfaceView implements SurfaceHolder.Callback
{
	...
	private float animationSteps = 1;
	...
	public void onDraw ( Canvas canvas )
	{
		...
		for ( PieSlice slice : slices )
		{
			...
			if ( count == 0 )
			{
				currentSweep = ( slice.getValue() / totalValue ) * ( 360 ) * ( step / animationSteps );
			}
			else
			{
				int add = ( int ) ( ( totalValue - slice.getValue() ) * ( 1 - ( step / animationSteps ) ) );
				currentSweep = ( ( slice.getValue() + add ) / totalValue ) * ( 360 );
			}
